	...
	*/
							...
							$.each(data,function(i){
								//console.log($(this).attr('parent_id'));
									...
									if( parent_id == null)
									{
										...
										if($(this).attr('name') == cont){
											contented = contented+'<option value='+self_id+' data-pid=5 selected>'+$(this).attr('name')+'</option>';
										}else{
										...
										if($(this).attr('name') == cont){
											contented = contented+'<option value='+$(this).attr('id')+' data-pid=5 selected>'+ optionName +'</option>';
										}else{
		...
		$('.add-table').click(function(){
			...
			aths.each(function(i){
				var title=$(this).html();
				if($(this).attr('data-hid') == 'true'){//在th里添加data-hid属性值为'true'时，该列不显示
					return true;
				}
				if(o==0){str+="<tr>";}
				if($(this).attr('class')=='options'){
					...
					$.ajax({
						...
						success:function(data){
							...
							contented = '<select name='+gid+' class=abc ><option value=0>请选择</option>';
			...
			inputs.each(function(i,obj){//待解决
				$(this).data('a',i)
			})
			inputs.blur(function(){
				var vals=$(this).val();
