        ...
        <div class="control-group span8">
          ...
          <div class="controls" >
            <select  name="allocatee">
              <?php if(is_array($staff)): foreach($staff as $key=>$staff): ?><option value="<?php echo ($staff["id"]); ?>" > <?php echo ($staff["name"]); ?> </option><?php endforeach; endif; ?>
            </select>
          </div>
        </div>
      </div>
      ...
      store = Search.createStore('distributedata',{
        proxy : {
          save : { //也可以是一个字符串，那么增删改，都会往那么路径提交数据，同时附加参数saveType
        	  addUrl : "",
              updateUrl : "<?php echo U('distribute');?>",
              removeUrl : "<<?php echo U('User/del');?>>"
          },
          method : 'POST'
        },
